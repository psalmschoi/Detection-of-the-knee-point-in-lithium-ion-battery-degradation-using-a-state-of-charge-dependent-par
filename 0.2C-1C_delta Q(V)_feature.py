...
for path_2 in subfolders:   
    ...
    for x in file_lis:
        ...
        df_new3=df_new[(df_new['Voltage']>=3.8)]
        mean=df_new3['Delta Q'].mean()
        ...
        data_folder='C:~'#The directory path of the folder where the data-saving file is located.
        excel_file=data_folder+'Mean_0.2C-1C_delta_Q(V).xlsx'#The Excel file address to save the data of Mean (Q0.2C-1C(V)) corresponding to each cell's RPT
        wb=xw.Book(excel_file)
        sht=wb.sheets['Mean']
        sht.range((row+3,num+1)).value=mean
        wb.save(excel_file)
        wb.app.quit() 
