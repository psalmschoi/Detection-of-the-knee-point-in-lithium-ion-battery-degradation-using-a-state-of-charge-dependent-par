...
def prepare_data(df, variable, target):
    ...
    y = np.log10(df[target].values.reshape(-1, 1) + 1)
    return X, y
...
def train_and_evaluate(X, y, alphas, iterations):
    ...
    for i in range(1, iterations + 1):
        ...
        y_list = np.hstack([X_test, np.power(10, y_test) - 1, np.power(10, y_pred_test) - 1])
...
if __name__ == "__main__":
    ...
    file_path = os.path.join(nowdir, 'deg-SL.xlsx')
    variable = 'Var (ΔQ0.2C-1C(V))'  # [Var (ΔQ0.2C-1C(V)), DCIR, Mean (ΔQ0.2C-1C(V))]
    target = 'deg-SL'
    df = pd.read_excel(file_path)
    X, y = prepare_data(df, variable, target)
    y_class = (np.power(10, y) - 1 >= 1).astype(int).ravel()
