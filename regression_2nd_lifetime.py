...
def MAPE(y_test, y_pred):
    y_test = np.power(10, y_test) #The target scale was too large, so the prediction was conducted using a log scale.
    y_pred = np.power(10, y_pred)
    return np.mean(np.abs((y_test - y_pred) / y_test) * 100)

def RMSE(y_test, y_pred):
    y_test = np.power(10, np.power(10, y_test)) #To express it in terms of cycle count, np.power(10,) is performed once more
...
if __name__ == "__main__":
    ...
    file_path = os.path.join(nowdir, '2nd_lifetime.xlsx')
    variable = 'Var (ΔQ0.2C-1C(V))'  # [Var (ΔQ0.2C-1C(V)), ΔQ_Δcycle, SOH, DCIR at low SOC(5%),Mean (ΔQ0.2C-1C(V))]
    target = 'cycle'
